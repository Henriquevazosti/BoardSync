import dbAdapter from './s    // 4. Verificar listas
    console.log('\n📝 LISTAS:');
    const lists = await dbAdapter.findMany('board_lists', {});
    console.log(`✅ ${lists.length} listas encontradas`);onfig/dbAdapter.js';
...
async function testIntegration() {
  try {
    ...
    // 4. Verificar listas
    console.log('\n� LISTAS:');
    const lists = await dbAdapter.findMany('lists', {});
    ...
    if (boards.length > 0) {
      ...
      const boardLists = await dbAdapter.findMany('lists', { board_id: board.id });
